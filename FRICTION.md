# Friction Log

Work that is harder than it should be — for a human or an AI. Low bar: if it feels like an
annoyance, log it.

| ID | Title | Status | Type | Date |
|----|-------|--------|------|------|
| FRICTION-001 | Enumerating a vendor MCP server needs a human in the loop | Open | Process overhead | 2026-08-26 |
| FRICTION-002 | Vendor documentation contradicts the vendor's own API | Accepted | Process overhead | 2026-08-26 |
| FRICTION-003 | The AI asked for resources it could have found itself | Resolved | AI-inefficient | 2026-08-26 |
| FRICTION-004 | The server has no login, and nothing says so where you look for one | Open | Discoverability | 2026-08-30 |
